...
import { getFirestore, collection, doc, setDoc, updateDoc, getDocs, getDoc, query, limit } from "https://www.gstatic.com/firebasejs/11.0.2/firebase-firestore.js";
...
async function fetchAndDisplayProducts(collectionName, containerId) {
    // Create a query to fetch only 2 documents
    const collectionRef = collection(db, collectionName);
    const limitedQuery = query(collectionRef, limit(2)); // Limit to 2 documents
    ...
    querySnapshot.forEach((doc) => {
        const data = doc.data();
        const productCard = `
        <a href="product-details.html?productId=${doc.id}&collection=${collectionName}">
        <div class="product">
...
// Fetch and display products from each collection
